...
class Connect_BBDD:
        ...
        with self.lock:
            if self.connection is None:
                self.connection = sqlite3.connect(
                    f"App_Data/{name}.db", check_same_thread=False
                )
                ...
                CTkMessagebox(
                    message="To create a new connection or database, close the current one.",
                    icon="warning",
                    option_1="Ok",
                )
                return
    ...
    def read_data(self, search, get_value):
        with self.lock:
            ...
            if get_value != "":
                ...
                CTkMessagebox(
                    message="Record is not found.", icon="warning", option_1="Ok"
                )
                return
    ...
    def verify_id(self, id):
        with self.lock:
            ...
            if self.output_id:
                CTkMessagebox(
                    message="ID already exists.", icon="warning", option_1="Ok"
                )
                return False
            else:
                return True
    ...
    def get_column(self):
        self.output_column = []
        self.cursor.execute(
            "SELECT lower(NAME) AS NAME , avg(VARIATION) FROM DATA GROUP BY lower(NAME)"
        )
    ...
    def get_tickers(self):
        ...
        self.tickers_list = [
            self.output_tickers[i][0] for i in range(len(self.output_tickers))
        ]
    ...
    def data_bases_list(self, directory):
        ...
        for file in os.listdir(directory):
            ...
            if os.path.isfile(self.path) and file.endswith(
                (".db", ".sqlite", ".sqlite3")
            ):
    ...
    def delete_data_base(self, name):
        if os.path.exists(name):
            ...
            name = name.split("/", 1)[1]
            CTkMessagebox(
                title="Info", message=f"Database {name} has been deleted.", icon="info"
            )

        else:
            name = name.split("/", 1)[1]
            CTkMessagebox(
                title="Info", message=f"Database {name} does not exist.", icon="info"
            )
            return
