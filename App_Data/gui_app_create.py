...
class Create_Data(ctk.CTkFrame):
    ...
    def set_frame_left(self, frame_create):
        ...
        self.id.trace_add(
            "write",
            lambda *args: self.id.set(self.id.get()[:30])
            if len(self.id.get()) > 30
            else None,
        )
        ...
        self.name.trace_add(
            "write",
            lambda *args: self.name.set(self.name.get()[:50])
            if len(self.name.get()) > 50
            else None,
        )

        # current = self.last_date_str.get() selected date = comparable date
        self.button_current_price = ctk.CTkButton(
            self.frame_left,
            text="Current Price",
            command=lambda: self.date_pick(
                0,
                340,
                self.entry_value,
                self.value,
        ...
        self.value.trace_add(
            "write",
            lambda *args: self.value.set(self.value.get()[:10])
            if len(self.value.get()) > 10
            else None,
        )

        self.button_last_price = ctk.CTkButton(
            self.frame_left,
            text="Last Price",
            command=lambda: self.date_pick(
                0,
                ...
                self.name.get(),
                self.last_date_str,
                current=self.current_date_str.get(),
            ),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.param.trace_add(
            "write",
            lambda *args: self.param.set(self.param.get()[:10])
            if len(self.param.get()) > 10
            else None,
        )
        ...
        self.button_ticker = ctk.CTkButton(
            self.frame_left,
            text="Ticker",
            command=lambda: st.create_ticker(
                self.name.get(),
                self.value,
                self.param,
                self.entry_comments,
    ...
    def set_frame_right(self, frame_create):
        ...
        self.button_read = ctk.CTkButton(
            self.frame_right,
            text="Get Data ",
            command=lambda: self.get_all_data(),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.button_search = ctk.CTkButton(
            self.frame_right,
            text="Search",
            command=lambda: self.search_id_name(
                self.optinomenu_var.get(), self.search.get()
            ),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.button_update = ctk.CTkButton(
            self.frame_right,
            text="Update",
            command=lambda: self.open_toplevel(self.row_selector.get()),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.button_delete = ctk.CTkButton(
            self.frame_right,
            text="Delete",
            command=lambda: self.ask_delete(self.row_selector.get()),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.button_delete = ctk.CTkButton(
            self.frame_right,
            text="Graph",
            command=lambda: self.open_toplevel_graph(),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        self.button_delete.grid(row=7, column=0, pady=10, padx=30)

        self.button_create_bbdd = ctk.CTkButton(
            self.frame_right,
            text="Connect",
            command=lambda: self.open_toplevel_connect(),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
    ...
    def deco_function(parameter_function):
        def internal_function(self, *args, **kwargs):
            ...
            if not status.connection_status():
                CTkMessagebox(
                    title="Info",
                    message="Please, create a connexion to database",
                    icon="info",
                )
                return
            else:
    ...
    def date_pick(
        self, x, y, entry_variable, get_date_stock, stock, current_last_date, **kwargs
    ):
        if stock == "":
            CTkMessagebox(
                title="Info", message="Please, insert a ticker code", icon="info"
            )
            return

        if (
            self.toplevel_window_picker is None
            or not self.toplevel_window_picker.winfo_exists()
        ):
            self.one_date_entry = entry_variable
            self.toplevel_window_picker = CTkDatePicker(
                self.one_date_entry, get_date_stock, stock, kwargs
            )

        self.toplevel_window_picker.toggle(
            x, y, entry_variable, stock, get_date_stock, current_last_date, kwargs
        )

    def center_top_frame(self, frame_create):
        self.frame_center_top = ctk.CTkScrollableFrame(
            frame_create,
        ...
        self.frame_center_top.pack(
            fill="both", expand=False, padx=5, pady=5, side="top"
        )

    def center_bottom_framel(self, frame_create):
        self.frame_center = ctk.CTkFrame(
            frame_create,
            fg_color=ctk.ThemeManager.theme["CTkFrame"]["fg_color"],
            width=500,
        )
    ...
    def center_bottom_framer(self, frame_create):
        self.frame_center_bottom = ctk.CTkFrame(
            frame_create,
            fg_color=ctk.ThemeManager.theme["CTkFrame"]["fg_color"],
            width=300,
        )
        self.frame_center_bottom.grid_propagate(False)
        self.frame_center_bottom.pack(
            fill="both", expand=False, padx=5, pady=5, side="right"
        )
        ...
        self.button_sort = ctk.CTkButton(
            self.frame_center_bottom,
            text="Sort",
            command=lambda: self.sort_table(self.radio_var.get()),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
    ...
    def on_resize(self, event):
        ...
        self._resize_after_id = self.after(
            200, lambda: self.table_output.update_idletasks()
        )

    def set_table(self, list_table, column_num):
        self.table_output = CTkTable(
            self.frame_center_top,
            column=column_num,
            values=list_table,
            anchor="center",
        ...
        self.row_selector = CTkTableRowSelector(
            self.table_output, can_select_headers=False, max_selection=True
        )
    ...
    def open_toplevel_connect(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = ToplevelWindowI(
                self
    ...
    def open_toplevel_graph(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = ToplevelWindowII(
                self, self
            )  # create window if its None or destroyed
    ...
    def get_data_list(self, id, name, value, param, comments):
        if self.validate_entry(
            num_id=id.get(),
            str_name=name.get(),
            num_current_price=value.get(),
            num_last_price=param.get(),
        ):
            self.variation = round(
                (float(value.get()) - float(param.get())) / float(param.get()) * 100, 3
            )
            self.comments = comments.get("1.0", "end-1c").strip()
            self.list_create_register = [
                id.get(),
                name.get(),
                float(value.get()),
                float(param.get()),
            ...
            def task():
                ...
                def gui_update():
                    if success:
                        msg = CTkMessagebox(
                            message="Data record has been inserted.",
                            icon="check",
                            option_1="Ok",
                        )
    ...
    def cleaner_data(self, *args):
        for i in args:
            if isinstance(i, ctk.CTkTextbox):
    ...
    def search_id_name(self, option, search):
        ...
        if self.validate_entry(**{option_menu: search}):
            ...
            try:
                ...
                CTkMessagebox(
                    title="Info",
                    message=f"Warning clearing selection: {e}",
                    icon="info",
                )
    ...
    def get_all_data(self):
        ...
        try:
            ...
            CTkMessagebox(
                title="Info", message=f"Warning clearing selection: {e}", icon="info"
            )
        # delete last table
        rows = len(self.table_output.get())
        for j in reversed(range(1, rows)):
    ...
    def update_data_table(self, name, value, param, comments, id):
        if self.validate_entry(
            num_id=id, str_name=name, num_value=value, num_param=param
        ):
            ...
            def task():
                ...
                def gui_update():
                    if success:
                        msg = CTkMessagebox(
                            message="Data record has been updated.",
                            icon="check",
                            option_1="Ok",
                        )
                        response = msg.get()
    ...
    def delete_data(self, row_selector_list):
        if row_selector_list:
            ...
            if delete_data_bbdd.delete_data(id_delete):
                msg = CTkMessagebox(
                    message="Data record has been deleted.", icon="check", option_1="Ok"
                )
                response = msg.get()
                if response == "Ok":
                    self.get_all_data()
    ...
    def ask_delete(self, row_selector_list):
        if row_selector_list:
            msg = CTkMessagebox(
                title="Warning",
                message=f"¿Do you want to delete the record {row_selector_list[0][0]} ?",
                icon="question",
                option_1="Cancel",
                option_2="No",
                option_3="Yes",
    ...
    def variation_ratio(self, list):
        if len(list) > 5:
            CTkMessagebox(
                title="Database Warning",
                message="Please select only five tickers.",
                icon="warning",
                option_1="Ok",
            )
            return
        ...
        for key, value in kwargs.items():
            if key.__contains__("num"):
                try:
                    ...
                    CTkMessagebox(
                        title="Database",
                        message=f"{key.split('_', 1)[1]}: Please enter a numeric value.",
                        icon="warning",
                        option_1="Ok",
                    )
                    print(f"Exception in validate_entry: {e}")
                    return
                ...
                try:
                    ...
                    CTkMessagebox(
                        title="Database Warning",
                        message=f"{key.split('_', 1)[1]}: Please enter some text.",
                        icon="warning",
                        option_1="Ok",
                    )
                    return
    ...
    def validate_entry_read(self, param, value):
        if param == "ID" and value != "":
            try:
                ...
                print(f"Exception in validate_entry_read: {e}")
                return False
...
class ToplevelWindow(ctk.CTkToplevel):
    ...
    def window_data_entry(self, data_instance):
        ...
        self.label_id = ctk.CTkLabel(
            self, text="ID", font=("Inter ExtraBold", 15), fg_color="transparent"
        )
        self.label_id.grid(row=1, column=0, padx=30, pady=10, sticky="ew")
        ...
        self.button_current_price = ctk.CTkButton(
            self,
            text="Current Price",
            command=lambda: data_instance.date_pick(
                0,
                340,
                self.entry_value,
                self.value,
        ...
        self.label_comments = ctk.CTkLabel(
            self, text="Comments", font=("Inter ExtraBold", 15), fg_color="transparent"
        )
        self.label_comments.grid(row=9, column=0, pady=10, padx=30, sticky="ew")
        ...
        self.button_create = ctk.CTkButton(
            self,
            text="Update",
            command=lambda: data_instance.run_in_thread(
                data_instance.update_data_table,
                self.name.get(),
                self.value.get(),
                self.param.get(),
                self.entry_comments.get("1.0", "end-1c").strip(),
                self.id.get(),
            ),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
        )
...
class ToplevelWindowI(ctk.CTkToplevel):
    ...
    def center_bottom_frame(self):
        ...
        self.frame_bottom_frame.pack(
            fill="both", expand=True, padx=5, pady=5, side="bottom"
        )
        ...
        self.button_connection = ctk.CTkButton(
            self.frame_bottom_frame,
            text="Connect",
            command=lambda: self.create_bbdd_connection(
                self.row_selector.get(), "", "connect"
            ),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.button_create_bbdd = ctk.CTkButton(
            ...
            text="Create",
            command=lambda: self.create_bbdd_connection(
                self.row_selector.get(), self.name.get(), "create"
            ),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        ...
        self.create_database = ctk.CTkLabel(
            self.frame_bottom_frame,
        ...
        self.entry_new_bbdd = ctk.CTkEntry(
            self.frame_bottom_frame, textvariable=self.name
        )
        self.entry_new_bbdd.grid(row=1, column=0, pady=5, padx=30, sticky="w")
        self.name.trace_add(
            "write",
            lambda *args: self.name.set(self.name.get()[:10])
            if len(self.name.get()) > 10
            else None,
        )

        self.button_delete_bbdd = ctk.CTkButton(
            self.frame_bottom_frame,
            text="Delete",
            command=lambda: self.delete_data_base(self.row_selector.get()),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
        self.button_delete_bbdd.grid(
            row=5, column=0, pady=(10, 15), padx=30, sticky="w"
        )
    ...
    def create_bbdd_connection(self, row_selector_bbdd, name, event):
        if row_selector_bbdd and event == "connect":
            ...
            CTkMessagebox(
                title="Database Warning",
                message="Please write a database name.",
                icon="warning",
                option_1="Ok",
            )
            return
    ...
    def delete_data_base(self, row_selector_bbdd):
        ...
        if row_selector_bbdd and self.get_db_status.db_selected_status(
            "App_Data/" + row_selector_bbdd[0][0]
        ):
            CTkMessagebox(
                title="Info",
                message="Please make sure to close the database before deleting it.",
                icon="info",
            )
            return
        ...
        if row_selector_bbdd:
            ...
            msg = CTkMessagebox(
                ...
                message=f"¿Do you want to delete the {row_selector_bbdd[0][0]} database?",
                icon="question",
                option_1="Cancel",
                option_2="No",
                option_3="Yes",
...
class CTkDatePicker(ctk.CTkToplevel):
    def __init__(self, master, set_date, stock, picked_date=None, **kwargs):
        ...
        # INIT VALUE
        # the calendar current date can be retrieved via self.calendar.current_date() when needed
        ...
        if hasattr(self.attach, "_canvas"):
            self.attach._canvas.tag_bind(
                "right_parts",
                "<Button-1>",
                lambda e: self.toggle(self.height_for_right, self.width_for_right),
            )
            self.attach._canvas.tag_bind(
                "dropdown_arrow",
                "<Button-1>",
                lambda e: self.toggle(self.height_for_arrow, self.width_for_arrow),
            )
        ...
        self.attach.winfo_toplevel().bind(
            "<Configure>", lambda e: self._auto_close(), add="+"
        )
    ...
    def _pass(self, date):
        ...
        for key, value in self.picked_date.items():
            if value == "":
                st.data_historical(
                    self.variable_stock, self.pick_date, self.variable_date
                )
    ...
    def date_verify_variation(self):
        for key, value in self.picked_date.items():
            ...
            if key == "last" and self.convert_picked_date > self.convert_pick_date:
                CTkMessagebox(
                    title="Info",
                    message="The date of the initial price (last price) must be earlier than the date of the final price.",
                    icon="info",
                )
                return False
...
class ToplevelWindowII(ctk.CTkToplevel):
    ...
    def center_bottom_frame(self, data_instance):
        ...
        self.button_create_bbdd = ctk.CTkButton(
            self.frame_bottom_frame,
            text="Graph",
            command=lambda: data_instance.variation_ratio(self.selected_tickers),
            font=ctk.CTkFont(family="Inter ExtraBold", size=15),
            width=100,
        )
    ...
    def tickers_option(self):
        ...
        for i in get_tickers.get_column():
            ...
            self.checkbox = ctk.CTkCheckBox(
                self.frame_center_top,
                text=ticker[0],
                command=lambda t=ticker: self.checkbox_event(t),
                variable=ticker_variable,
                onvalue="on",
                offvalue="off",
            )
